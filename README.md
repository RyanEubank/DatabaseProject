...
- Go to advanced system settings -> view advanced system settings -> advanced tab -> enviromental variables -> user variables -> new -> set JAVA_HOME as the variable name -> set path of where JDK is installed as variable value -> ok -> ok -> apply  (apparently it's path enviroment varible where it points ot that binary (which i assume to be the jdk?)
