...
In the launch.bat file, be sure to edit the correct path to your javafx-sdk-19/lib folder if you are not using the javafx libraries already provided. The code in the batch file can also be run directly in the command prompt from the root directory of the project. Navigate to the correct folder using the cd command, ex: $cd \<local path\>/DatabaseProject. Run the following commands to compile and launch the program:
```
